...
import { postBuyDataPlan } from "../../lib/api/url";
...
class BuyAirtime extends React.Component {
    ...
    state = {
        ...
        phoneNumber: '',
        networkOptions: [
            {label: "MTN DATA", value: 'MTNDATA'},
            {label: "AIRTEL DATA", value: 'AIRTELDATA'},
            {label: "ETISALAT DATA", value: 'ETISALATDATA'},
            {label: "GLO DATA", value: 'GLODATA'},
        ],
        network: '',
        plan: '',
        walletTypeOptions: [
    ...
    onSubmit = () => {
        ...
        let data = {
            ...
            "network_provider_data": this.state.network.value,
            'data_service_code':this.state.plan.code
        }

        this.setState({
            loading: true
        }, () => {
            apiRequest(postBuyDataPlan, 'post', data).then(res => {
    ...
    validate = () => {
        ...
        if (this.state.plan === '') {
            this.setState({
                planError: "Please select a data plan",
            })
            error = true;
        }
        return error
    ...
    onChangeNetwork = (item) => {
        if(this.props.dataPlans[item.value]){
            this.setState({network: item, networkError: '', amount:'',plan:''})
        }
        else{
            this.props.fetchDataPlanOptions(item.value)
            this.setState({network: item, networkError: '', amount:'',plan:''})
        }
    ...
    render() {
        ...
        let dataOptions = this.state.network? this.props.dataPlans[this.state.network.value]: [];
        ...
        return (
            <View style={{flex: 1, backgroundColor: '#fff'}}>
                <Header title={"Buy Data"} leftIcon={"ios-arrow-back"}
                        onPressLeftIcon={() => this.goBack()}/>
                <LoaderText visible={this.state.loading} desciption={'Purchasing Data'}/>
                    ...
                    <View style={{marginTop: 0, padding: scale(20), width: '100%', marginBottom: scale(30)}}>
                            ...
                            <View>
                                ...
                                <View>
                                    ...
                                    <SelectPopup
                                        ...
                                        title={'Select a network provider'}
                                ...
                                <View>
                                    <Text style={styles.label}>Select Data Plan Option</Text>
                                    <SelectPopup
                                        options={dataOptions|| []}
                                        title={'Select a Data Plan Option'}
                                        value={this.state.plan ? this.state.plan : {label: ''}}
                                        textStyle={{
                                            color: '#484848',
                                            fontFamily: 'graphik-regular',
                                            marginRight: scale(3),
                                        ...
                                        style={styles.pickerInput}
                                        onChange={(item) => {
                                            this.setState({plan: item, planError: '', amount:item.amount})
                                        }}
                                    />
                                    {
                                        !!this.state.planError && (
                                            <Text style={styles.error}>{this.state.planError}</Text>
                                        )
                                    }
                                </View>
                        ...
                        <View>
                            ...
                            >
                                ...
                                {(!this.state.loading) && (
                                    <ButtonWithBackgroundText>{'Buy Data'}</ButtonWithBackgroundText>)}
                            </ButtonWithBackgroundBottom>
